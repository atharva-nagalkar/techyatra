...
// GET /api/food - list restaurants with filters
router.get('/', async (req, res) => {
  try {
    const { state, city, cuisines, minPrice, maxPrice, minRating, search, limit = 20, page = 1 } = req.query;

    const filter = { isActive: true };
    if (state) filter.state = new RegExp(state, 'i');
    if (city) filter.city = new RegExp(city, 'i');
    if (minRating) filter.rating = { $gte: parseFloat(minRating) };
    if (cuisines) filter.cuisines = { $in: cuisines.split(',').map(c => new RegExp(c, 'i')) };
    ...
    const items = await Restaurant.find(filter).skip(skip).limit(parseInt(limit)).sort({ rating: -1, createdAt: -1 });
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!item) return res.status(404).json({ success: false, error: 'Restaurant not found' });
